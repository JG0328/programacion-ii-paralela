...
int main(int argc, char *argv[])
{
    ...
    if (argc < 2)
    {
        if (rank == 0)
        {
            cout << "No se ha especificado numero de elementos, multiplo de la cantidad de entrada, por defecto sera "
                 << size * 100;
    ...
    vector<long> VectorA, VectorALocal, VectorBLocal;
    ...
    // Generar Vector B local
    for(int i = 0; i < tama/size; i++)
    {
        int valor = ((rank * 10) + 1) + i;
        VectorBLocal[i] = valor * 10;
    }
    //
